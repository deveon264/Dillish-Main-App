...
## Expo Go on a physical phone can't reach the dev server on Replit — use --tunnel
Replit's dev domain proxies the workflow's `localPort` only via its `externalPort` (here `localPort 5000 → externalPort 80`), so `https://<domain>` works but `https://<domain>:5000` does NOT. Expo Go fetches the manifest fine over the proxy, but then requests the JS bundle at `https://<domain>:5000/...entry.bundle` (Expo embeds the Metro listen port) → red "Could not connect to development server". Setting `REACT_NATIVE_PACKAGER_HOSTNAME` only fixes the host, not the embedded `:5000` port, so it doesn't help. A phone on cellular is also off-LAN, so the `exp://172.x:5000` LAN URL is unreachable too.
**Why:** there's no way to expose arbitrary port 5000 on the dev domain, so the bundle URL's port can never match.
**How to apply:** Run the dev server with `--tunnel` (e.g. `npx expo start --tunnel --web --port 5000`). It creates an ngrok URL like `exp://<id>-anonymous-5000.exp.direct` (real connection over 443, no port mismatch) for both manifest and bundle. Pre-install the tunnel dep first (`npm install -D @expo/ngrok@^4.1.0`) — in a non-interactive workflow `--tunnel` can't prompt to auto-install it. Keep `--web` so the canvas iframe (localhost:5000 via the proxy) still renders. Confirm via logs: "Tunnel ready." + an `exp://...exp.direct` URL, and an `iOS Bundled ...` line once a device connects. The user must rescan the NEW QR code (the old one points at the broken `:5000` URL).
