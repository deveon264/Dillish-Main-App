...
// constants/workouts.ts `require()`s real .png/.jpg asset bytes (handled by
...
// The stub above makes every image a { uri: "/abs/path/to/asset.png" }.
...
test("matches on workoutId + workoutExerciseId", () => {
  ...
  assert.equal(fileOf(image), "the-hundred.png");
});
...
test("falls back to workoutExerciseId alone when no/wrong workoutId", () => {
  ...
  assert.equal(fileOf(noWorkout), "downward-dog.png");
